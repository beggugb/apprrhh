...
const SearchContratos = ({getComponent}) => {
    ...
    const { total, indicador, modalView, estado }= useSelector(state => state.contratos)
    ...
    const onAprobar = () => {                  
      ...
      dispatch(crudActions.SET_APROBAR('CONTRATOS_DATA','contratos/aprobar',xcode,'lista'))      
    ...
    return ( 
      ...
      <Row>
        ...
        <ButtonGroup>
          ...
          <Button className={indicador === 0 || estado === true ? "bts bg-default text-white disabled": "bts bg-default text-white"} onClick={()=> getComponent('edit',indicador)}>
            <FontAwesomeIcon icon={faFileExport} /></Button>                                                 
          <Button className={indicador === 0 || estado === false ? "btr bg-default text-white disabled": "btr bg-default text-white"} onClick={()=> viewArticulo()}>
            <FontAwesomeIcon icon={faFilePdf} /> </Button>   
          <Button className={indicador === 0  || estado === true ? "bts bg-default text-white disabled": "bts bg-default text-white"} onClick={()=> toggleModalViewx()}>
      ...
      <Modal isOpen={viewx} toggle={toggleModalViewx} className="deleteBody">  
          ...
          <Row>
              <Col md="12" >
              <p className="deletePe">desea aprobar el contrato ?</p>  
              </Col>              
            </Row>
            <Row className="mt-3">
